...
GolombCompressed golomb_compress(const std::vector<std::int64_t>& sorted_arr,
                                 int div_param) {
  if (sorted_arr.empty()) {
  ...
  std::int64_t div = div_param >= 0
                    ? static_cast<std::int64_t>(div_param)
                    : static_cast<std::int64_t>(std::max(
                          0.0, std::round(-std::log2(-std::log2(1.0 - prob)))));

  std::string compressed;
  std::int64_t res_idx = 0;
  ...
  while (it != sorted_arr.end()) {
    ...
    if (start | (curr > prev)) {
      ...
      auto remainder = delta & ((static_cast<std::int64_t>(1) << div) - 1);
      ...
      compressed[unary_end / CHAR_SIZE] |=
          static_cast<char>(static_cast<std::int64_t>(1) << (unary_end % CHAR_SIZE));

      auto binary_start = (unary_end + 1) % CHAR_SIZE;
      std::int64_t binary_idx = 0;
      std::int64_t i = (unary_end + 1) / CHAR_SIZE;
      ...
      while (binary_idx < div) {
        compressed[i] |= static_cast<char>(
            (static_cast<std::uint64_t>(remainder) >> binary_idx) << binary_start);
...
std::vector<std::int64_t> golomb_intersect(
    const std::string& golomb_compressed, std::int64_t div,
    const std::vector<std::pair<std::int64_t, std::int64_t>>& sorted_arr) {
  ...
  while (true) {
    ...
    // portion
    auto ctz = static_cast<std::int64_t>(CTZ(
    ...
    while (binary_idx < div) {
      ...
      remainder |= (static_cast<std::int64_t>(static_cast<unsigned char>(*it) >>
                                         binary_start) &
                    ((static_cast<std::int64_t>(1) << num_bits) - 1))
                   << binary_idx;
      binary_idx += num_bits;
      binary_start = 0;
      ++it;
